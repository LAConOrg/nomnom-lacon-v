...
# build this image: docker build --target dev -t lacon-v:dev .
# run it: docker run --rm -it --env-file .env --env-file .env.docker --network lacon-v_default --name nomnom-dev lacon-v:dev
FROM run AS dev
